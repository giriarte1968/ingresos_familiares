...
**RO-UI-01 (RESTABLECER TODAS FORZA RECÁLCULO PREVIEW):** El botón "↩️ Restablecer todos" reselecciona todos los checkboxes, limpia `comp_excluded`, y SÍ setea `forzar_recalculo=True` para que el header muestre el valor natural del pool completo (preview consistente). Sin embargo, NO persiste ni comitea el cambio — el usuario debe hacer clic en "✅ Aplicar selección" para persistir. Ver test `test_ui_reset_all_visual_only`, y `STATUS_ACTUAL.md §8`.
...
**RO-UI-04 (INHIBICIÓN DE RESTAURACIÓN EN PREVIEW FRESCO):** Cualquier acción que dispare un recálculo forzado para previsualización (`preview_mode=True` AND `forzar_recalculo=True`) DEBE inhibir la restauración automática de exclusiones desde la UV. La restauración solo es permitida en re-entry pasivo o cuando el usuario hace clic en "✅ Aplicar Selección". Ver test `test_ui_retro_toggle_inhibits_exclusion_restoration`, función `_should_restore_excl()` en `valu.py`.

**RO-UI-05 (TOGGLE RETRO/FLEX RESETEA SELECCIÓN):** Cualquier toggle de Retro (ON/OFF), Flex (ON/OFF) o cambio en el slider de meses resetea la selección de comparables a su estado por defecto (todos seleccionados). Esto es intencional: el pool de comparables cambia con estos parámetros y la selección previa podría referenciar IDs que ya no existen en el nuevo pool. Se muestra un mensaje `st.info()` informando al usuario. Ver handlers en `valu.py` líneas 378-427.

**RO-HEADER-04 (HEADER SOLO CAMBIA CON EXCLUSIÓN DE COMPARABLES):** El header solo debe mostrar el valor recalculado del preview cuando el usuario ha aplicado una exclusión de comparables (`_comp_exclusion_applied=True` o `comp_excluded_` en session_state). Cambios de Retro/Flex/Slider sin exclusión activa NO cambian el header — el header mantiene el valor oficial hasta que el usuario hace clic en "✅ Aplicar selección". Ver funciones `_tiene_exclusion_activa()` y `_should_show_preview_header()` en `valu.py`, test `test_header_no_cambia_con_retro_sin_aplicar`.
