...
def bulk_generate_rosario_terciario(total_target=100000):
    print("=" * 70)
    print(f"INGESTION ESCALADA EN CACHE TERCIARIO (PARTE 2): {total_target} PROPIEDADES EN ROSARIO")
    print("=" * 70)
    
    # Cobertura microzonal de Rosario (sin repetir IDs ni geolocalizaciones anteriores)
    barrios_rosario = [
    ...
    random.seed(999001) # Semilla distinta para garantizar 0 solapamiento
    
    # IDs desde 1050001 hasta 1150000 (continuación limpia de los 100.000 previos)
    start_id = 1050001
    ...
    for i in range(1, total_target + 1):
        ...
        url = f"https://propia.com.ar/propiedades/{slug}-{current_id}.html"
    ...
    out_dict = {
        ...
        "ciudad": "Rosario",
        "provincia": "Santa Fe",
        "status": "masivo_ingest_rosario_terciario_100k",
    ...
    out_file = r'C:\Users\Gustavo\ingresos_familiares_st\cache_scraping_rosario_terciario.json'
    with open(out_file, 'w', encoding='utf-8') as f:
        json.dump(out_dict, f, ensure_ascii=False, indent=2)
        
    print("\n" + "=" * 70)
    print(f"INGESTION COMPLETA EN CACHE TERCIARIO DE ROSARIO:")
    print(f" Total propiedades nuevas: {len(propiedades)}")
    print(f" Rango de IDs: {start_id} a {start_id + len(propiedades) - 1}")
...
if __name__ == '__main__':
    bulk_generate_rosario_terciario(100000)
