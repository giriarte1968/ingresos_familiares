        ...
        with col_limpiar:
            ...
            if pendiente:
                ...
                if st.button("🔄 Limpiar", type="secondary", use_container_width=True, key=f"cln_comps_{prop_name}"):
                    # --- NUCLEAR CLEAN: Direct File I/O to avoid any helper bugs ---
                    try:
                        import json
                        import os
                        
                        # 1. Cache Clear
                        cache_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'valuaciones_cache.json')
                        if os.path.exists(cache_path):
                            with open(cache_path, 'r', encoding='utf-8') as f:
                                cache_v = json.load(f)
                            cache_v.pop(prop_name, None)
                            with open(cache_path, 'w', encoding='utf-8') as f:
                                json.dump(cache_v, f, indent=2, ensure_ascii=False)
                        
                        # 2. Disk Properties Clear
            ...
            if not ya_valuado:
                ...
                if resultado_cacheado and cache_preview:
                    ...
                    if not forzar and not cache_valido:
                        ...
                        # Forzar engine run como preview para reemplazar envenenado (TAREA-111)
                        forzar = True
                        preview_mode = True
                        st.session_state[f'preview_mode_{p_obj["nombre"]}'] = True
                        print(f"[DEBUG-FLOW] {p_obj['nombre']}: PREVIEW ENVENENADO LIMPIADO — forzando engine run (preview)")
                    else:
...
""", unsafe_allow_html=True)
            try:
                with profile_block("detalle_spinner_valuar", p_obj):
                    ...
                    uv = p_obj.get('_ultima_valuacion', {})
                    if ya_valuado and not forzar:
                        stored_retro = uv.get('retro_dias', 0)
                        stored_flex = uv.get('flex_dormitorios', 1)
                        
                        # Respect active session state overrides if set during current view, else use stored UV
                        retro_active = st.session_state.get(f'retro_active_{prop_name}', (stored_retro > 0))
                        if not retro_active:
                            retro_dias = 0
                        else:
                            retro_dias = st.session_state.get(f'retro_meses_{prop_name}', stored_retro or 60)
                            
                        flex_active = st.session_state.get(f'flex_active_{prop_name}', (stored_flex is not None and stored_flex != 1))
                        if not flex_active:
                            flex_dormitorios = 1
                        else:
