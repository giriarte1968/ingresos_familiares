...
## 📅 2026-05-27 — TAREA-012: Regla estricta de fuente de año para comparables

### Objetivo
Reemplazar la lógica de `enriquecer_anio_comparable()` con reglas estrictas: años scraping → ALTA confianza, años AVM → MEDIA confianza solo con matching riguroso (misma calle+número exacto o misma calle+≤20m).

### Cambios

#### `parsers/mercado_inmobiliario.py`

1. **`normalizar_calle_nombre(direccion)`**: lowercase, sin tildes, sin puntuación, Av/Bv removido, prefijos honoríficos eliminados (Almirante→Brown, General→Paz, San→Martín, etc.), espacios colapsados.
2. **`extraer_calle_numero(direccion)`**: extrae (calle_normalizada, numero_int) de direcciones libres. Maneja "al 2100", pisos, ordinales "4º", intersecciones.
3. **`obtener_anio_scraping(comp)`**: busca `anio_construccion`→`anio_estimado`→`year`→`antiguedad`. Solo retorna ALTA si año válido (1900-2027).
4. **`enriquecer_anio_comparable()` reescrita**: scraping→ALTA; sin lat/lon→None; Regla A (calle+número exactos)→MEDIA; Regla B (calle+≤20m)→MEDIA; resto→None.
5. **Call site**: contadores `n_anio_source_scraping/avm/none`, metadata `anio_source/anio_confianza/anio_match_tipo/anio_distancia_match/anio_ph_match/anio_direccion_catastro`.
6. **Fix**: `radio_usado: None` crash en `generar_razonamiento_valuacion()` → `if radio is None: radio = 300`.

#### `tests/test_age_enrichment.py` (nuevo, 26 tests)
- Normalización, extracción, scraping detection, AVM strict rules.

### Impacto en regression tests (5/39 FAIL — esperado)
| Test | Antes ($) | Después ($) | Causa |
|------|-----------|-------------|-------|
| `test_mabel_venta` | ~79k | ~88,920 | 3/79 comps con AVM años (3.8%) |
| `test_patio_grande_vera` | ~52k | ~39,531 | Pocos años → sin age filter → P33 |
...
### Decisión
- `max_dist_m` reducido de 50 a 20m para Regla B
- `pct_con_anio` esperado muy bajo (~3.8% en Mabel) — la tarea prioriza confianza sobre cobertura
- Regression tests NO deben actualizarse sin autorización humana

### Archivos
- `parsers/mercado_inmobiliario.py` — 3 helpers nuevos + enriquecer_anio_comparable reescrita
- `tests/test_age_enrichment.py` — nuevo, 26 tests
- `docs/ALGORITMOS.md` — Sección "13. Fuentes de año de comparables"
- `docs/STATUS_ACTUAL.md` — Sección 15
- `docs/DICCIONARIO_DATOS.md` — campos nuevos en comparable
- `docs/BITACORA_AGENTES.md` — esta entrada
- `.opencode/plans/TAREA-012.md` — plan archivado
- `.opencode/plans/TAREAS_INDEX.md` — índice actualizado
