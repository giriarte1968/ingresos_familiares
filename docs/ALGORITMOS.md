...
## 13. Fuentes de año de comparables (TAREA-012)

### Jerarquía de fuentes

Cada comparable recibe un año de construcción mediante `enriquecer_anio_comparable()`:

### Scraping → ALTA (sin validación adicional)

Si el comparable tiene año en campos del scraping (`anio_construccion` → `anio_estimado` → `year` → `antiguedad`), se asigna ALTA confianza inmediatamente. No requiere matching geográfico ni AVM.

### Sin scraping → AVM solo con match estricto → MEDIA

Si el comparable no tiene año scraping pero tiene coordenadas, se busca en el AVM catastral (`rosario_avm_full.csv`):

1. **Regla A — Calle + número exactos** (confianza MEDIA)
   - Se normaliza el nombre de calle del comparable vs cada candidato AVM.
   - Si la calle normalizada coincide Y los números son iguales → año del AVM, confianza MEDIA.
   - Sin límite de distancia (solo el bbox 0.001° ~111m del pre-filtro).

2. **Regla B — Misma calle + distancia ≤20m** (confianza MEDIA)
   - Si la calle normalizada coincide pero el número difiere (o no hay número), y la distancia entre puntos es ≤20m → año del AVM, confianza MEDIA.

3. **Sin match** → None (el comparable no tiene año).

### Normalización de calles

`normalizar_calle_nombre(direccion)` realiza:

| Transformación | Ejemplo | Resultado |
|---------------|---------|-----------|
| lowercase | "Brown" | "brown" |
| sin tildes | "Güemes" | "guemes" |
| sin puntuación | "Av. del Valle" | "av del valle" |
| Av/Avenida → removido | "Av. del Valle" → "del valle" | "del valle" |
| Bv/Bulevar → removido | "Bv. Oroño" → "oroño" | "oroño" |
| Honoríficos → removido | "Almirante Brown" | "brown" |
| "General Paz" | "paz" |
| "San Martín" | "martin" |
| Colapsar espacios | " brown  " | "brown" |

### Extracción de número

`extraer_calle_numero(direccion)` extrae el primer número de la dirección:

| Entrada | Calle normalizada | Número |
|---------|------------------|--------|
| "Brown 2734" | "brown" | 2734 |
| "Av. Pellegrini 1200" | "pellegrini" | 1200 |
| "Almirante Brown al 2100" | "brown" | 2100 |
| "Corrientes 1166 Piso 6" | "corrientes" | 1166 |
| "Alvear y Rivadavia" | "alvear y rivadavia" | None |

### Metadata expuesta

Cada comparable en `comparables_reales` incluye:
- `anio_source`: 'scraping' | 'avm' | None
- `anio_confianza`: 'ALTA' | 'MEDIA' | None
- `anio_match_tipo`: 'campo_scraping' | 'calle_numero_exactos' | 'calle_distancia_20m' | None
- `anio_distancia_match`: metros al AVM matched (solo AVM)
- `anio_ph_match`: PH del CSV catastral (solo AVM)
- `anio_direccion_catastro`: dirección del CSV catastral (solo AVM)
