...
def test_ui_apply_button_visible_when_all_selected():
    """TAREA-120: Con selección completa (6/6) y fresh entry (sin apply previo),
    el botón debe mostrar 'Aplicar selección' (active), NO 'Selección Aplicada'.
    Solo 'Selección Aplicada' aparece después de hacer click en Apply.
    """
    import streamlit as st
    from unittest.mock import patch, MagicMock
    ...
    with patch('streamlit.columns', side_effect=_cols_side_effect), \
        ...
        assert called_with_apply, (
            f"Fresh entry con selección completa debe mostrar 'Aplicar selección' (active). "
            f"Llamadas a st.button: {mock_btn.call_args_list}"
        )
        assert not called_with_applied, (
            f"NO debe mostrar 'Selección Aplicada' en fresh entry (nunca se hizo click). "
            f"Llamadas a st.button: {mock_btn.call_args_list}"
        )
...
def test_exclusion_applied_flag_cleared_after_retro_toggle():
    ...
    fresco no tiene el flag. Con flags limpiados, el botón vuelve a 'Aplicar selección'.
    """
    import streamlit as st
    from unittest.mock import patch, MagicMock
    from valu_detail_sections import render_tabla_comparables, _get_comp_id
    ...
    with patch('streamlit.columns', side_effect=_cols_side_effect_with_checkbox), \
        ...
        assert len(apply_calls) >= 1, (
            f"Con flags limpiados tras Retro toggle, debe mostrar 'Aplicar selección'. "
            f"Calls: {mock_btn.call_args_list}"
        )
        assert len(applied_calls) == 0, (
            f"NO debe mostrar 'Selección Aplicada' cuando los flags fueron limpiados. "
...
def test_ct_runtime_generar_resultado_manual():
    ...
    from parsers.location_engine import cargar_anclas
    anclas = cargar_anclas()
    acevedo_fisherton = next((a for a in anclas if a['id'] == 'acevedo_fisherton'), None)
    assert acevedo_fisherton is not None, "acevedo_fisherton debe existir"
    raw_val = acevedo_fisherton['usd_m2_raw']
    effective_val = acevedo_fisherton['usd_m2']
    assert raw_val > 0, f"acevedo_fisherton usd_m2_raw debe > 0, got {raw_val}"
    assert effective_val > 0, f"acevedo_fisherton usd_m2 debe > 0, got {effective_val}"
...
def test_clean_no_hay_zombies_fuera_del_boton():
    ...
    for lineno in collector.lines:
        assert 390 <= lineno <= 950, \
            f"RO-CLEAN-04: pendiente_comparables=True en línea {lineno}, " \
            f"fuera del rango esperado (390-950). Posible zombie."
