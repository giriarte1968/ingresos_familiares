"""
Hybrid IDW+P33 Simulation — with ±10yr age filter.
"""
import json
import math
import sys
...
def compute_year(antiquity):
    """Convert antiquity to year. antiquity is years since construction."""
...
valid_year = sum(1 for p in venta if p['year'])
print(f"  With valid year: {valid_year} ({valid_year/len(venta)*100:.1f}%)")
...
def idw_p33_dorm_exact_hb(comps, subj_dorms, subj_m2, lat, lon, subj_year=None, age_window=10):
    """IDW-weighted P33 + exact dorm + hard barriers + age filter"""
    if not comps:
        return None, 0
    ...
    # Age filter: ±10 years
    if subj_year:
        year_min = subj_year - age_window
        year_max = subj_year + age_window
...
def idw_p33_dorm_exact_hb_f50(comps, subj_dorms, subj_m2, lat, lon, subj_year=None, age_window=10):
    """IDW-weighted P33 + exact dorm + hard barriers + age filter + floor 50m"""
    if not comps:
        return None, 0
...
print("\n" + "="*100)
print("HYBRID IDW+P33 SIMULATION — WITH ±10yr AGE FILTER")
print("="*100)
...
for prop in propiedades:
    ...
    if year and year < 100:  # antiquity, not year
    ...
    m5, m5_n = idw_p33_dorm_exact_hb(comps, dorms, m2, lat, lon, year, 10)
    m6, m6_n = idw_p33_dorm_exact_hb_f50(comps, dorms, m2, lat, lon, year, 10)
    
    # Also compute WITHOUT age filter for comparison
    m5_noage, m5_noage_n = idw_p33_dorm_exact_hb(comps, dorms, m2, lat, lon, None, 10)
    
    # Count age-filtered comps
    age_comps = 0
    if year:
        year_min = year - 10
        year_max = year + 10
        age_comps = sum(1 for c in comps if c['year'] and year_min <= c['year'] <= year_max
                       and not crosses_barrier(lat, lon, c["lat"], c["lon"], -32.965)
                       and not crosses_barrier(lat, lon, c["lat"], c["lon"], -32.933))
    ...
    results.append({
        ...
        "m5": m5, "m5_n": m5_n,
        "m6": m6, "m6_n": m6_n,
        "m5_noage": m5_noage, "m5_noage_n": m5_noage_n,
        "age_comps": age_comps,
    })
...
print(f"\n{'Property':<22} {'m2':>5} {'D':>2} {'Year':>5} | {'Current':>9} {'No-Age':>9} {'+Age':>9} {'+Age+F50':>9} | {'AgeC':>5} {'Total':>5}")
print("-"*110)
...
for r in results:
    ...
    print(f"{r['name']:<22} {r['m2']:>5} {r['dorms']:>2} {yr:>5} | {fmt(r['m1']):>9} {fmt(r['m5_noage']):>9} {fmt(r['m5']):>9} {fmt(r['m6']):>9} | {r['age_comps']:>5} {r['n_total']:>5}")
...
methods = ["m5_noage", "m5", "m6"]
labels = ["IDW-DE-HB (no age)", "IDW-DE-HB (+10yr age)", "IDW-DE-HB-F50 (+10yr+floor)"]
...
print("\n" + "="*100)
print("DETAILED EXAMPLES — AGE FILTER IMPACT")
print("="*100)
...
for idx, r in enumerate(results):
    ...
    # Filter same dorm + hard barriers
    same_dorm_hb = [c for c in comps if (c["dormitorios"] or 2) == r["dorms"]
                    and not crosses_barrier(lat, lon, c["lat"], c["lon"], -32.965)
                    and not crosses_barrier(lat, lon, c["lat"], c["lon"], -32.933)]
    ...
    if year:
        year_min = year - 10
        year_max = year + 10
        age_filtered = [c for c in same_dorm_hb if c['year'] and year_min <= c['year'] <= year_max]
    
    print(f"\n--- {name} (m2={r['m2']}, dorm={r['dorms']}, year={int(year) if year else 'N/A'}) ---")
    print(f"  Same-dorm + HB: {len(same_dorm_hb)}, After ±10yr age filter: {len(age_filtered)}")
    
    if year:
        print(f"  Year window: {int(year-10)} to {int(year+10)}")
    
    # Show age-filtered comps
    if age_filtered:
        print(f"\n  Age-filtered comparables:")
        print(f"  {'#':>3} {'Dist':>6} {'P33':>8} {'D':>2} {'m2':>5} {'Year':>5}")
        print(f"  {'-'*35}")
        
        for i, c in enumerate(age_filtered[:10]):
            yr = int(c['year']) if c['year'] else "?"
            print(f"  {i+1:>3} {c['dist_m']:>5.0f}m ${c['valor_m2_adj']:>7.0f} {c['dormitorios']:>2} {c['m2']:>5.0f} {yr:>5}")
    
    # Results
    print(f"\n  Results:")
    print(f"    Current:               ${r['m1']:.0f}/m2")
    print(f"    IDW-DE-HB (no age):    ${r['m5_noage']:.0f}/m2 ({r['m5_noage_n']} comps)")
    print(f"    IDW-DE-HB (+10yr):     ${r['m5']:.0f}/m2 ({r['m5_n']} comps)")
    print(f"    IDW-DE-HB-F50 (+10yr): ${r['m6']:.0f}/m2 ({r['m6_n']} comps)")
